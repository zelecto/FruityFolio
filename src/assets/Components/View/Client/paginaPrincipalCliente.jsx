...
export const ViewPaginaPrincipalClient = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const respuesta = await GetTiendasCiudad("Barranquilla");
...
const CardTienda = ({ tienda, colorFondo, imgTopProductos = [] }) => {
    ...
    return (
        <div style={{ backgroundColor: colorFondo }} className="h-[350px] w-full rounded-lg flex justify-between font-serif shadow-xl">
            <div className="p-5 flex flex-col justify-between h-full ">
                ...
                <button onClick={handleExplorarClick} className="w-80 flex font-semibold text-xl border-b-2 border-black items-center">
        ...
        <div className="relative w-full overflow-hidden ">
            <div className="flex transition-transform ease-out duration-500"
                ...
                {images.map((image, index) => (
                    <div className=" w-full  rounded-md flex-shrink-0 flex justify-center items-center" key={index}>
                        <img
            ...
            <button onClick={prev} className="absolute top-1/2 transform -translate-y-1/2  p-2 rounded-full shadow-md">
                <CircleChevronLeft size={20}></CircleChevronLeft>
            </button>
            <button onClick={next} className="absolute top-1/2 right-0 transform -translate-y-1/2 p-2 rounded-full shadow-md">
